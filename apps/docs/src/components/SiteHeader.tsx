    ...
    <header className="cfg__bar">
      <div className="cfg__bar-in">
        <a className="cfg__mark" href="/" aria-label="Lucet home">
          ...
          {page !== 'configurator' ? <span className="cfg__page">· {page}</span> : null}
        </a>

        {children}

        <nav className="cfg__nav" aria-label="Site">
          {PAGES.filter((p) => p.id !== page && (!p.dev || import.meta.env.DEV)).map((p) => (
            <a className="cfg__navlink" key={p.id} href={p.href}>
              {p.label}
            </a>
          ))}
          <a
